...
export function getAudioPaths(lang = 'en') {
  ...
  const universalBase = 'audio/';
  const result = {};
  for (const [key, file] of Object.entries(audioFileNames)) {
    // guess_success and guess_error are language agnostic
    const pathBase = (key === 'guess_success' || key === 'guess_error')
      ? universalBase
      : base;
    result[key] = pathBase + file;
  }
  return result;
}
